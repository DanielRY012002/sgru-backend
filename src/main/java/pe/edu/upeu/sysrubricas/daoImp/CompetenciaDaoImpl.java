...
public class CompetenciaDaoImpl implements CompetenciaDao {
	...
	public int update(Competencia c) {
		// TODO Auto-generated method stub
		return jdbcTemplate.update("call D_CRUD_COMPETENCIAS.SPP_update_COMPETENCIAS(?,?)", c.getId_competencias(),
				c.getDescripcion());
	}
	...
	public Map<String, Object> read(int id) {
		...
		// .declareParameters(new SqlOutParameter("cursor_competencias",
		// OracleTypes.CURSOR, //esto solo para procedimientos
		// new ColumnMapRowMapper()), new SqlParameter("id_competencia",
		// Types.INTEGER)); //integer:tipo entero
		// SqlParameterSource in = new
		// MapSqlParameterSource().addValue("id_competencia", id);

		simpleJdbcCall = new SimpleJdbcCall(jdbcTemplate).withCatalogName("d_crud_competencias") // nombre del paquete
